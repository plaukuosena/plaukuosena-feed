...
Stable version: local sitemap.xml -> online sitemap -> existing kaina24.xml fallback.
"""
...
def fetch_text(url: str, timeout: int = 25) -> str:
    ...
    if response.status_code == 403:
        ...
        response = session.get(url, headers=alt_headers, timeout=timeout, verify=False)
...
def load_source_urls(cfg: dict[str, Any]) -> tuple[list[str], str]:
    local_sitemap = ROOT / "sitemap.xml"
    if local_sitemap.exists():
        try:
            sitemap = local_sitemap.read_text(encoding="utf-8")
            urls = sitemap_urls(sitemap)
            product_urls = [url for url in urls if likely_product_url(url, cfg)]
            if product_urls:
                return product_urls, "local_sitemap"
            print("WARN local sitemap exists but no product URLs matched filters", file=sys.stderr)
        except Exception as exc:
            print(f"WARN local sitemap failed: {exc}", file=sys.stderr)
    try:
        sitemap = fetch_text(cfg["store"]["sitemap_url"])
        urls = sitemap_urls(sitemap)
        product_urls = [url for url in urls if likely_product_url(url, cfg)]
        if product_urls:
            return product_urls, "online_sitemap"
    except Exception as exc:
        print(f"WARN online sitemap failed: {exc}", file=sys.stderr)
    fallback_urls = product_urls_from_kaina24_xml()
    return fallback_urls, "fallback_existing_xml"
...
def infer_brand(title: str, cfg: dict[str, Any], ld_brand: Any = None) -> str:
    ...
    known_brands = [
        "WELLA PROFESSIONALS", "Wella Professionals", "American Crew", "AMERICAN CREW",
        "MILK SHAKE", "Milk Shake", "Kadus Professional", "Kadus Professionals", "KADUS PROFESSIONALS",
        "OLAPLEX", "Olaplex", "DAVINES", "Davines", "QIQI", "Qiqi",
        "L'Oréal Professionnel", "Loreal Professionnel", "L'Oreal Professionnel",
    ...
    for brand in known_brands:
        if brand.lower() in low_title:
            return brand
    first_word = clean_title.split(" ")[0].strip()
    return first_word if first_word else ""


def infer_size(text: str) -> str:
    match = re.search(r"(\d+(?:[,.]\d+)?)\s?(ml|g|l|kg|vnt|pcs)\b", text, flags=re.I)
    if not match:
        return ""
...
def model_code(title: str, brand: str, type_code: str) -> str:
    ...
    return "-".join(tokens[:2]) if tokens else "GEN"
...
def generate_sku(title: str, brand: str, size: str, cfg: dict[str, Any]) -> str:
    ...
    return re.sub(r"-+", "-", f"{brand_code}-{model}-{type_code}-{size_code}").strip("-")
...
def clean_price(value: Any) -> str:
    if value is None:
        return ""
    text = str(value).replace("€", "").replace("EUR", "")
...
def extract_ean_from_page(page_html: str) -> str:
    ...
    label_match = re.search(r"\bEAN\b(.{0,2500}?)(\d{8,14})", plain_text, flags=re.I | re.S)
    if label_match:
        return label_match.group(2)
    html_match = re.search(r"EAN.{0,5000}?(\d{8,14})", decoded, flags=re.I | re.S)
...
def parse_product(url: str, cfg: dict[str, Any]) -> Product | None:
    ...
    description = normalize_text(ld.get("description") if isinstance(ld, dict) else "")
    if not description:
    ...
    return Product(sku, title, brand, price, "", currency, url, image, category, availability, size, description[:500], ean_code, "OK" if price else "CHECK_PRICE")
...
def write_csv(products: list[Product], path: Path) -> None:
    fields = list(asdict(products[0]).keys()) if products else ["sku", "title", "brand", "price", "old_price", "currency", "url", "image", "category", "availability", "size", "description", "ean_code", "source_status"]
...
def write_xlsx(products: list[Product], path: Path) -> None:
    ...
    ws = wb.active
    ws.title = "Products"
    headers = ["sku", "title", "brand", "price", "currency", "url", "image", "category", "availability", "size", "ean_code", "source_status"]
    ws.append(headers)
    for product in products:
    ...
    widths = {"A": 22, "B": 48, "C": 22, "D": 10, "E": 10, "F": 55, "G": 45, "H": 22, "I": 14, "J": 14, "K": 18, "L": 16}
...
def main() -> int:
    cfg = load_config()
    product_urls, source_mode = load_source_urls(cfg)
    print(f"Source mode: {source_mode}; product URLs: {len(product_urls)}")
    if not product_urls:
        raise RuntimeError("No product URLs found from local sitemap, online sitemap, or fallback XML.")
