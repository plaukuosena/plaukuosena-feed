...
def write_csv(products: list[Product], path: Path) -> None:
    fields = list(asdict(products[0]).keys()) if products else [
        "sku", "title", "brand", "price", "old_price", "currency", "url",
        "image", "category", "availability", "size", "description", "source_status"
    ]
...
def write_xlsx(products: list[Product], path: Path) -> None:
    ...
    ws.title = "Products"
    headers = [
        "sku", "title", "brand", "price", "currency", "url", "image",
        "category", "availability", "size", "source_status"
    ]
    ws.append(headers)
    for product in products:
    ...
    widths = {
        "A": 22, "B": 48, "C": 22, "D": 10, "E": 10, "F": 55,
        "G": 45, "H": 22, "I": 14, "J": 12, "K": 16
    }
